...
See [this page](http://gtk-rs.org/docs-src/requirements) for information on how to install GTK+.
...
impl Widget<Msg> for Win {
    ...
    fn update(&mut self, event: Msg, model: &mut Model) {
        match event {
            // A call to self.label1.set_text() is automatically inserted by the attribute every time the model.counter attribute is updated.
    ...
    view! {
        Window {
            gtk::Box {
                ...
                Label {
                    // By the text property of this Label to the counter attribute of the model.
                    // Every time the counter attribute is updated, the text property will be updated too.
            ...
            // Use a tuple when you want to both send a message and return a value to the GTK+ callback.
